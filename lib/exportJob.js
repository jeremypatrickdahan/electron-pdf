...
class ExportJob extends EventEmitter {
  ...
  _getBrowserConfiguration (args) {
    ...
    const webSecurity = _.get(this.options, 'webSecurity', true)
    const useSessions = _.get(this.options, 'useSessions', true)
  ...
  _captureWindow (context, generateFn, outputDoneFn) {
    ...
    const injectedJavascript = _.get(this.options, 'injectedJavascript', null)
    if(injectedJavascript){
      window.webContents.once('did-finish-load', ()=>{
        window.webContents.executeJavaScript(injectedJavascript)
      }) 
    }
  ...
  _capturePDF (args, window, done, outputFile) {
    ...
    const useCallbacks = (process.versions.electron < '7')
    debugLogger(pdfOptions)
    try {
      if(args.usePrinter){
        if(useCallbacks){
          window.webContents.print(pdfOptions, (success)=>{
            if(success){
          ...
          window.webContents.print(pdfOptions)
          .then(data => {
            this.emit('pdf-complete', {})
            debugLogger(`success printing.`)
          })
          .catch(error => {
            this._emitResourceEvents(error, data, done)
        ...
        window.webContents.printToPDF(pdfOptions, (error, data)=>{
          if(error){
            debugLogger(`error printing PDF:`, error)
  ...
  _getTargetFile (context) {
    ...
    if (_.size(this.input) > 1) {
      return this.output.substring(0, extIndex) + `_${zeroBasedIndex + 1}${actualExt}`
    }
